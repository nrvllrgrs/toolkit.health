...
{
	[CustomEditor(typeof(HealthComposite))]
    public class HealthCompositeEditor : Editor
    {
		#region Fields

		protected HealthComposite m_healthComposite;
		...
		protected HealthGroupTreeView m_treeView = null;
		...
		{
			m_healthComposite = target as HealthComposite;
			m_healthComposite.UpdateGroups();
			m_healthComposite.UpdateGroupAssignments();
		...
		public override void OnInspectorGUI()
		{
			...
			if (m_healthComposite.objects.Length > 0)
			{
				if (m_treeView == null)
				{
					...
					m_treeView = new HealthGroupTreeView(
						new TreeViewState(),
		...
		private void OnSceneGUI()
		{
			...
			foreach (var row in m_treeView.GetRows())
			{
				...
				if (!m_healthComposite.TryGetGroup(obj, out var group))
					break;
		...
		{
			if (m_healthComposite == null)
				return;

			if (forced || !m_visualize.boolValue)
			{
				foreach (var renderer in m_healthComposite.GetComponentsInChildren<Renderer>())
				{
	...
	public class HealthGroupTreeView : TreeView
	{
		#region Fields

		private HealthComposite m_model = null;
		...
		public HealthGroupTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader, HealthComposite model)
